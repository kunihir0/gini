= OSX-Forge: QEMU/KVM Deployment System Architecture
:toc: left
:toclevels: 3
:sectlinks:
:sectnums:
...
│   ├── kernel/                # Minimal application kernel
│   │   ├── mod.rs
│   │   ├── bootstrap.rs       # Core bootstrap
...
=== Core Kernel
The OSX-Forge kernel is an ultra-lightweight component responsible for:

* Loading and initializing plugins
* Managing the plugin lifecycle
* Providing the stage manager interface
* Establishing the minimal communication bus
* Managing system storage access

All actual functionality is implemented as plugins, including core functionality.
