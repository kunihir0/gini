...
impl Plugin for ConfigUsingPlugin {
    ...
    fn init(&self, app: &mut Application) -> KernelResult<()> {
        ...
        // Get the storage manager from the application
        ...
        // Update configuration to record usage
        let mut updated_config = config.clone();
        updated_config.set("last_used", SystemTime::now().elapsed().unwrap_or_default().as_secs())?;
        
        // Save updated configuration as user-specific override
        storage_manager.save_plugin_config(
            self.name(), 
            &updated_config, 
...
async fn test_plugin_configuration_management() {
    ...
    let user_config = storage_manager.load_config(&plugin_name, scope)
    ...
    let merged_config = storage_manager.get_plugin_config(&plugin_name)
